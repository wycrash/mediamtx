	...
	if m.Variant == MuxerVariantMPEGTS {
		for _, track := range m.Tracks {
			if track.Codec.IsVideo() {
				...
				default:
					return fmt.Errorf(
						"the MPEG-TS variant of HLS supports MPEG-4 Audio or MPEG-1/2 Audio only")
				}
				hasAudio = true
			}
		}
...
func (m *Muxer) WriteH264(
	...
	return m.segmenter.writeH264(m.mtracksByTrack[track], ntp, pts, 0, false, au)
}

// WriteH264WithDTS writes an H264 access unit using an explicit PES DTS.
// Use this when remuxing MPEG-TS with B-frames so DTS is not reconstructed from POC.
func (m *Muxer) WriteH264WithDTS(
	track *Track,
	ntp time.Time,
	pts int64,
	dts int64,
	...
	return m.segmenter.writeH264(m.mtracksByTrack[track], ntp, pts, dts, true, au)
}

// WriteMPEG1Audio writes MPEG-1/2 Audio frames (MPEG-TS variant only).
...
func (m *Muxer) rotateSegmentsInner(
	...
	// Refuse MPEG-TS segments shorter than the configured minimum.
	if m.Variant == MuxerVariantMPEGTS && m.leadingStream != nil {
		if seg, ok := m.leadingStream.nextSegment.(*muxerSegmentMPEGTS); ok && seg != nil {
			if nextDTS-seg.startDTS < m.SegmentMinDuration {
				return nil
			}
		}
	}
