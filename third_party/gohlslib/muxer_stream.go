...
type muxerStream struct {
	...
	mpegtsTimeBase         int64             // mpegts only, first DTS/PTS subtracted on write
	mpegtsTimeBaseInit     bool              // mpegts only
	mpegtsLeadingDTS       int64             // mpegts only, last leading-video DTS (track clock)
	mpegtsLeadingDTSInit   bool              // mpegts only
	segments               []muxerSegment
...
// mpegtsRebase shifts timestamps so the muxed PCR/PTS/DTS timeline starts near zero.
// Segment cutting still uses the original clock; only PES/PCR values are rebased.
func (s *muxerStream) mpegtsRebase(pts int64, dts int64) (int64, int64) {
	if !s.mpegtsTimeBaseInit {
		s.mpegtsTimeBase = dts
		s.mpegtsTimeBaseInit = true
	}
	return pts - s.mpegtsTimeBase, dts - s.mpegtsTimeBase
}

func (s *muxerStream) setLeadingDTS(dts int64) {
	s.mpegtsLeadingDTS = dts
	s.mpegtsLeadingDTSInit = true
}
